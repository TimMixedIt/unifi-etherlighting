...
Der Versionswert liegt an `$.version`. Der Device-Read verlangt `$.meta.rc = ok` und ein Array an `$.data`. Der Write verlangt HTTP-Erfolg und `$.meta.rc = ok`; der Brightness-Service prüft zusätzlich den Responsewert und einen unabhängigen Read-after-Write.
